...
COPY --chown=appuser:appuser . /home/appuser/app/api

# Required in Windows and MAC OS to run the entrypoint.sh script
RUN sed -i 's/\r$//' /home/appuser/app/api/entrypoint.sh \
    && chmod 744 /home/appuser/app/api/entrypoint.sh

RUN mv /home/appuser/app/api/entrypoint.sh /usr/local/bin/entrypoint.sh
